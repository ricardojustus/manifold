...
- **The bright-line list stays short.** Every addition trades adaptive judgment for mechanical certainty; the three-part test in ENFORCEMENT.md gates additions. When in doubt, prose.
- **Right-size everything — YAGNI with a floor** (`principles/right-sized-engineering.md`): before building machinery, verify the need is real, nothing (especially the platform's native layer) already provides it, and the tradeoff wasn't already litigated — waivers and stated postures are inherited, not re-derived per audit round. Process weight scales with stakes × reversibility. The receipt is this repo's own audit: three rounds spent hardening a declared-best-effort seatbelt hook, resolved by radical simplification and a waiver. The floor: YAGNI never trims bright-line invariants, block-path tests, receipts, or small-but-real needs — unsure means ask, not drop.

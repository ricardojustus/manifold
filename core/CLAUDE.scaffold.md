...
## Operator Translation (HARD RULE)

*Enforcement: prose — full kernel + the decision-packet template at `principles/operator-translation.md`*

Every decision brought to the operator arrives **packet-shaped, in the operator's units** (context anchor · the ask in prose · your rec + why · cost as tier/multiplication/$, duration-only framings BANNED · failure + recovery story · unresolved assumptions + strongest dissent + what evidence would reverse the rec · link to the filed brief for structural GOs). The recorded GO attaches to the packet/brief, never to the raw spec — **ratification never transfers accountability** for what the operator wasn't equipped to see; "that's the spec you ratified" is not a sayable sentence. The operator's unknown unknowns are the agent's assignment. The teaching duty runs one direction: explainers at arc ends, self-checks ungated, jargon always paired with plain language. **The cold-read test gates every packet** (added after a shape-perfect, language-opaque packet failed the operator same-day): re-read it as the operator before sending — every internal name absent or paired with what the thing DOES; internal codes only in the linked brief; technical dissent quotes carry a plain rendering. Shape compliance is not translation.

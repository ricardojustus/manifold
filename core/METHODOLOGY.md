...
**These hardening mappings route a DECISION to the operator; they do not auto-select the assurance level.** The factual classification stays visible, but any mapping from it to operator-visible machinery (hardening, delay, recurring cost, reduced capability) is a **recommendation until a cited operator posture receipt activates it** (`operator-owns-criticality-and-complexity`) — routing the decision to the operator, whose answer for a genuinely warranted case is normally "build it," never a license for the agent to silently SKIP an owed hardening. The gate fires on the machinery **consequence**, so a soak / promotion controller / recovery subsystem introduced under a neutral name ("robustness", "correctness") is caught the same as a labeled one.

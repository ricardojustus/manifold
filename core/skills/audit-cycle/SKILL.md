...
name: audit-cycle
description: >-
  Dispatch the pre-merge layer-audit cycle — a primary reviewer subagent + an independent cross-model reviewer (each with a completion-watcher), multi-round to the lock-gate floor (default 0/0/0 C/H/M) with structured Low triage and MAX-severity consolidation. THE GATE (operator ruling 2026-07-14): ALL spec-implementing work — a branch implementing a LOCKED spec or contract, a spec-LOCK cycle, a per-layer audit in that lane — enters this ladder, PERIOD. Non-spec work (quick fixes, ops) defaults to the methodology's light review and enters at the builder's judgment. Audit subject = the implementation AS A WHOLE, not the diff. Trigger phrases: "audit X" / "round-N audit" / "audit before merge" / "fix-pass" / "consolidate the audits" / "/audit-cycle" — and proactively when a spec-lane branch is ready to merge. NOT: a scoped security pass (`scoped-adversarial-audit`), subsystem inventory (`system-audit`), or single-PR diff review (the runtime's PR tool).
---
...
- Non-spec work (quick fixes, ops, tooling) — the methodology's **light review** by default (self-review + exercise-the-change + regression test where it's a bugfix + a single fresh-eyes reviewer at the builder's judgment), entering this ladder at the **builder's judgment** (operator ruling 2026-07-14: all SPEC-implementing work is audited, period — and non-spec work is never auto-excluded; the builder decides, and during autonomous work records the call in the run journal).
...
**Content-policy refusal = a terminal signal with a MODEL-SWAP remedy (not a retry).** An adversarial, security-framed review can trip the cross-model provider's own content filter: the analysis runs, then the provider REFUSES to emit the final report (a cyber-risk / dual-use filter reading "attack this code" as security work). Treat a refusal signature in the returned result as terminal for that attempt — a same-model retry re-trips the same filter. Remedy: re-dispatch the SAME prompt once on the binding's named **filter-free fallback model** (the binding pins the exact string + the refusal signatures to detect); the swap counts against the dispatch attempt cap. Fallback also refused, or no fallback configured → the single-lens degradation path above, recorded honestly. Never soften a refusal into "the reviewer found nothing" — a refused review is a MISSING lens, not a clean one. *Receipt 2026-07-12: a security review completed its analysis and the provider refused the final report ("flagged for possible cybersecurity risk"); the fixtures existed, the findings didn't.*

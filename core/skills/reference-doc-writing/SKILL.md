...
description: Write a current-state reference doc — the Diátaxis reference genre — for a subsystem: a stable lookup surface describing what's actually running right now. Covers the canonical section structure (module map / flows / invariants / extension points / log surface / debugging recipe / what's NOT in scope / pointers), the tone conventions (factual, imperative for procedures, explicit "as of <date>" stamp), the tier-by-stability rule (write when a subsystem has stabilized, not while it churns), and the spec→reference promotion close-out (fold an implemented spec's durable behavior into the owning subsystem's doc, same session, with a provenance pointer). Use when the operator says "write the reference doc", "document this subsystem", "/reference-doc-writing", or proactively when a subsystem stabilizes or an implemented spec needs promotion. NOT tutorials/how-tos/explanations (different genres); NOT deciding where a doc belongs (use `doc-placement`); NOT design intent (use `plan-update`).
---

# Reference doc writing

A current-state reference doc describes **what's actually running, right now**, as a stable lookup surface for future-you debugging or a future agent onboarding. It's a different genre from plan docs (design intent) and lesson files (hard-won lessons). This skill is that genre specifically.

The shape below has been validated repeatedly across subsystems — the same structure each time. Replicate it. (The project binding names the concrete example docs to read for shape, plus the doc-index/registry the project uses.)

## Diátaxis context

The Diátaxis framework (https://diataxis.fr) splits technical docs into four genres:

| Genre | Purpose | Audience mode |
|---|---|---|
| **Tutorial** | Learn by doing | Study |
| **How-to guide** | Solve a specific problem | Work |
| **Reference** | Look up facts | Work |
| **Explanation** | Understand concepts | Study |

This skill is about the **reference** genre: information-oriented, precise, structured for lookup (not narrative). Writing something that's actually a tutorial (step-by-step walkthrough) or an explanation (concept deep-dive)? Different genre, different home — doesn't go in the current-state reference corpus.
...
Don't preemptively write a reference doc for something still in flux — it goes stale faster than you can update it, and it misrepresents current state as stable when it isn't.

Signals a subsystem is ready: shipped to production; survived at least one incident without architectural rework; a couple of weeks without substantial change; you're being asked the same debugging questions repeatedly (the doc replaces verbal re-explanation).

## Spec → reference promotion (close-out)

A second, event-triggered reason to write/update a reference doc — independent of the tier clock: **when a spec finishes implementation, its durable behavior promotes into a reference doc as part of definition-of-done, in the SAME implementing session** (not deferred doc-debt). This is the loop that stops knowledge loss at implementation time. Procedure (full taxonomy in `doc-placement`):
...
- **Add the forward pointer.** The reference doc is the only *searchable* surface, so it's the discovery path to the un-indexed provenance — add, in the promoted section, a provenance comment: `<!-- provenance: spec <archived-spec-path>; adr <adr-path> -->` (drop the `adr` clause if none was written).
...
This does NOT conflict with the tier rule. The tier rule bars *preemptive* docs (documenting unshipped design that will churn). An *implemented* spec is different: its behavior has shipped, so it IS current state and is documentable now — fold it in this session, scoping the section to what shipped plus an honest "not yet in scope" for the rest. If you genuinely can't write a coherent subsystem-shaped section for it, that's a signal the spec isn't actually done — resolve that, don't defer the promotion.
...
Use ASCII flow diagrams for significant execution paths — dense, grep-friendly, and they survive markdown renderers better than prose:
...
What the platform guarantees as a contract — the "if you violate these, it's our bug" list:

```markdown
- **Atomic state writes**: rename-based — no half-written files on crash.
- **No read-modify-write races**: a per-path lock serializes reads+modifies+writes.
- **No silent success lies**: success state is set only when the underlying action succeeded.
```
...
Honest about scope gaps — what the subsystem explicitly does NOT handle, so extenders + debuggers don't assume it does. List them, then a one-line "none of these block <the next thing>".
...
- **Concrete over abstract.** Real command names, file paths, event kinds, config keys — generic descriptions age poorly.
...
- **Each `##`/`###` section answers ONE "how does X work" question, self-contained.** A reader landing on just that section should get a complete answer. Don't write "as described above" / "see the previous section" — restate the anchor or link it.
- **Stable, descriptive headings.** The heading text becomes the chunk's anchor; renaming it breaks inbound `path#anchor` citations. Prefer "Boot sequence", "Write scope" over generic "Details" / "Notes". Name headings to last.
- **Structured over prose.** Tables, code blocks, and explicit pattern lists retrieve + parse better than narrative paragraphs.
- **Cite source concretely.** `file:line` / exact symbol / event-kind names ground the retrieval; generic descriptions age poorly.

---
name: council
description: Convene the Round Table — a fresh, ephemeral panel of up to 2 strong-reasoning + 2 cross-model adversarial seats that reviews a Vision (and Plan) from first principles, and returns ONE consolidated severity-rated findings list. This is the methodology's Council for **Gate A** (vision-only challenge) and **Phase 5** (vision + plan lock). Invoke when the user says "convene the council" / "council review" / "round table" / "run Gate A" / "Phase 5 council" / "/council", or proactively when a Vision (Gate A) or a Vision+Plan (Phase 5) of a Medium/High-stakes build is ready for adversarial review BEFORE locking. ADVISORY ONLY — it never edits an artifact, never locks, never loops back; the Orchestrator + Human disposition its findings. This is NOT a code/spec audit: for per-artifact pre-merge gating use `audit-cycle`; for security-surface passes use `scoped-adversarial-audit`; for subsystem inventory use `system-audit`. The Council reviews INTENT and DESIGN, not a diff.
---

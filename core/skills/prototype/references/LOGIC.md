...
Before writing code, write down what state model and what question you're prototyping. One paragraph, at the top of the demo (in a visible intro, not just a comment). A logic prototype that answers the wrong question is pure waste — make the question explicit so it can be checked later, whether the operator is watching now or returning to it AFK.

### 2. Isolate the logic in a portable module

Put the actual logic — the bit that's answering the question — in a single `<script>` block written as a small, pure module whose DESIGN could carry into the real codebase later. The page around it is throwaway; the module's design is the part worth keeping.
...
This is what makes the prototype useful past its own lifetime: once the question's answered, the validated design — the state shape, the transitions, the interface — is what carries into the real build.
...
Once the prototype has answered its question, capture the answer, then capture the prototype the way the [SKILL](../SKILL.md) describes. The logic-specific mapping: the validated design (the reducer shape, the machine's states and transitions, the function signatures) is the decision that carries forward — the real implementation re-enters through the project's normal build gates (spec, implementation, review), written to production standard there. The prototype file, logic module included, stays on the throwaway branch as the primary source; prototype code never ships. Being one self-contained file, it stays trivially re-runnable there.
...
- **Don't blur the logic and the page together.** If the pure module references the DOM, `document`, or button handlers, it's no longer portable. Keep the page as a thin shell over a pure module.
...
- **Don't ship the demo into production** — neither the HTML shell nor the logic module behind it. The page is optimised for being clicked through by hand, and the module was written to answer a question, not to production standard. The module's *design* is the bit worth keeping.

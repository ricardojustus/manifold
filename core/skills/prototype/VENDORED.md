...
- **Vetting**: full end-to-end read of the upstream skill dir (SKILL.md + LOGIC.md + UI.md +
  `agents/openai.yaml` interface stub; no scripts), 2026-07-30. The adopting project keeps the
...
3. `LOGIC.md` and `UI.md` moved to `references/` (harness layout), each with the `SKILL.md` →
   ...
   code-into-production clauses in BOTH: LOGIC.md's lift-the-module steps (3 and 7) and UI.md's
   capture step (6: fold-the-winner / promote-the-variant) + both files' closing anti-patterns
   (LOGIC's worth-keeping wording at :79, UI's promotion wording).
